...
document.addEventListener('DOMContentLoaded', function(){
    ...
    var imageScour = document.querySelectorAll('[id="lb-target"]');
    ...
    for (var i=0; i<len; i++) {
        ...
        // Make the clicktags and attach them to clickable image

        clickTag = document.createElement('a');

        clickTag.setAttribute('id', i);
        clickTag.setAttribute('href', '#_');
        clickTag.setAttribute('class', 'lightbox-link');

        imgClone = lightboxImages[i].cloneNode();
        imgParent = lightboxImages[i].parentNode;

        clickTag.appendChild(imgClone);

        imgParent.replaceChild(clickTag, lightboxImages[i]);
